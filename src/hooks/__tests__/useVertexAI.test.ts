...
import { useVertexReport, useVertexChat } from '../useVertexAI';
...
describe('useVertexReport', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('returns initial state with null data and streaming=false', () => {
    const { result } = renderHook(() => useVertexReport());
  ...
  it('sets error when stream throws', async () => {
    mockStreamReport.mockReturnValue(
      asyncGenError([], new Error('Vertex AI request failed (401): Unauthorized')),
    );
    ...
    expect(result.current.error).toBe('Vertex AI request failed (401): Unauthorized');
  });
...
describe('useVertexChat', () => {
  ...
  it('appends user message immediately when send is called', async () => {
    mockStreamChat.mockReturnValue(asyncGen(['Hello', ' there!']));
  ...
  it('progressively updates model response as stream yields chunks', async () => {
    mockStreamChat.mockReturnValue(asyncGen(['Your ', 'win rate ', 'is 65%.']));
  ...
  it('sets error when chat stream throws', async () => {
    mockStreamChat.mockReturnValue(
      asyncGenError([], new Error('Vertex AI chat request failed (500): Server Error')),
    );
    ...
    expect(result.current.error).toBe('Vertex AI chat request failed (500): Server Error');
  ...
  it('passes context to streamChat', async () => {
    mockStreamChat.mockReturnValue(asyncGen(['response']));
    ...
    expect(mockStreamChat).toHaveBeenCalledWith(
      expect.any(Array),
      'Summary: 100 trades, 65% win rate',
