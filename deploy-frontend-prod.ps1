<#
.SYNOPSIS
    Build and deploy TradeQut frontend to PROD (tradequt.com)
#>

param(
    [string]$AwsCli = "C:\Program Files\Amazon\AWSCLIV2\aws.exe",
    [string]$Bucket = "tradequt.com-prod",
...
Write-Host "`n=== Deploying to PROD (tradequt.com) ===" -ForegroundColor Cyan

# Step 1: Build
Write-Host "`n[1/3] Building for production..." -ForegroundColor Yellow
bun run build
...
Write-Host "`n=== PROD deployment complete ===" -ForegroundColor Green
Write-Host "Site: https://tradequt.com" -ForegroundColor White
